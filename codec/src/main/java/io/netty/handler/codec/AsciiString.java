...
public final class AsciiString implements CharSequence, Comparable<CharSequence> {
    ...
    public AsciiString(byte[] value, boolean copy) {
        ...
        valueStart = 0;
        valueLen = valueEnd = value.length;
    }
     ...
     */
    public AsciiString substring(int start) {
    ...
    public AsciiString subSequence(int start, int end) {
        ...
        return new AsciiString(value, ix(start), end - start, false);
    }
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = valueStart, j = that.valueStart; i < valueEnd; i ++, j ++) {
    ...
    public String toString() {
        ...
        for (int i = 0, j = valueStart; i < length; i ++, j ++) {
            array[i] = (char) (value[j] & 0xFF);
        }

        return this.string = new String(array);
    }
    ...
    public String toString(int start, int end) {
        ...
        char[] array = new char[length];
        for (int i = 0, j = ix(start); i < length; i ++, j ++) {
            array[i] = (char) (value[j] & 0xFF);
        }
    ...
    public int compareTo(CharSequence string) {
        ...
        for (int i = valueStart, j = 0; j < minLength; i ++, j ++) {
    ...
    public int compareToIgnoreCase(CharSequence string) {
        ...
        if (string instanceof AsciiString) {
            ...
            for (int i = 0, j = valueStart; i < minLength; i ++, j ++) {
                byte v1 = value[j];
                byte v2 = that.byteAt(i);
                if (v1 == v2) {
                    continue;
                }
            ...
            for (int i = 0, j = valueStart; i < minLength; i ++, j ++) {
                int c1 = toLowerCase(value[j]) & 0xFF;
    ...
    public AsciiString concat(CharSequence string) {
        ...
        if (string instanceof AsciiString) {
            ...
            System.arraycopy(value, valueStart, newValue, 0, thisLen);
            System.arraycopy(that.value, that.valueStart, newValue, thisLen, thatLen);
    ...
    public boolean equalsIgnoreCase(CharSequence string) {
        ...
        for (int i = valueStart, j = 0; i < valueEnd; i ++, j ++) {
    ...
    public byte[] toByteArray(int start, int end) {
        return Arrays.copyOfRange(value, ix(start), ix(end));
    }
    ...
    public void copy(int srcIdx, ByteBuf dst, int dstIdx, int length) {
        ...
        dst.setBytes(dstIdx, value, ix(srcIdx), length);
    }
    ...
    public void copy(int srcIdx, ByteBuf dst, int length) {
        ...
        dst.writeBytes(value, ix(srcIdx), length);
    }
    ...
    public void copy(int srcIdx, byte[] dst, int dstIdx, int length) {
        ...
        System.arraycopy(value, ix(srcIdx), dst, dstIdx, length);
    }
    ...
    public void copy(int srcIdx, char[] dst, int dstIdx, int length) {
        ...
        for (int i = ix(srcIdx), j = dstIdx; j < dstEnd; i ++, j ++) {
    ...
    public int lastIndexOf(int c, int start) {
        if (start >= 0) {
            ...
            for (int i = ix(start); i >= valueStart; -- i) {
                if ((value[i] & 0xFF) == c) {
                    return i;
                }
            }
    ...
    public AsciiString trim() {
        int start = valueStart, last = valueEnd;
        int end = last;
    ...
    public AsciiString[] split(char delim) {
        ...
        if (start == 0) { // If no delimiter was found in the value
            ...
            if (start != valueEnd) {
                ...
                res.add(new AsciiString(value, start, valueEnd - start, false));
            } else {
